				...
				<TableBodyRow on:dblclick={() => goto(`/${entityLower}/${tableRow.acct}`)}>
					{#each Object.values(tableRow).slice(1, columns + 1) as value, index}
						...
						>
							...
							{:else if value === false}
								🚫
							{:else if tableRow.photoURL && value.includes('https')}
								<img class=" w-10 h-auto max-w-xs" src={tableRow.photoURL} alt="User" />
							{:else if tableRow.status !== 'failed' && tableRow.domainName && typeof value === 'string' && value?.includes('https')}
